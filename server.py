...
# ---------- Local Emergent Integrations Replacement ----------
class ImageContent:
...
class LlmChat:
    def __init__(self, api_key: str, session_id: str, system_message: Optional[str] = None):
        ...
        self.base_url = "https://demobackend.emergentagent.com/v1"
    def with_model(self, provider: str, model: str):
        self.provider = provider
        self.model = "gemini-1.5-flash" if "gemini" in provider else model
        return self
    async def send_message(self, message: Union[UserMessage, str]) -> str:
        if isinstance(message, str):
            message = UserMessage(text=message)
        
        # Simplified message format that is more likely to work with Gemini proxies
        ...
        async with httpx.AsyncClient(timeout=60.0, follow_redirects=True) as client:
            try:
                ...
                text = resp.text
                if not text:
                    raise Exception(f"AI API returned an empty response (Status: {resp.status_code})")
                
                if resp.status_code != 200:
                    raise Exception(f"AI Error {resp.status_code}: {text}")
                
                try:
                    data = resp.json()
                    return data.get("content", "") or data.get("message", "") or text
                except:
...
async def _llm_json(system: str, user_text: str, session_id: str, image_b64: Optional[str] = None) -> Any:
    # Optimized AI connection for FridgeChef AI
    chat = LlmChat(
    ...
    try:
        ...
        logger.error(f"AI Failure: {str(e)}")
        # If the direct API fails, try the proxy endpoint
        try:
            chat.base_url = "https://api.emergentagent.com/v1"
            resp = await chat.send_message(msg)
            return _extract_json(resp)
        except:
            # Last resort: Try the legacy endpoint
            try:
                chat.base_url = "https://api.emergentagent.com/llm/v1"
                resp = await chat.send_message(msg)
                return _extract_json(resp)
            except:
                raise e
...
# ---------- AI Endpoints ----------
SCAN_SYSTEM = """You are FridgeChef AI's ingredient detection AI. You look at a photo of a fridge, pantry, groceries, vegetables, or leftovers and identify visible edible ingredients.

Return ONLY valid JSON in this exact schema (no prose, no markdown):
{
  "ingredients": [
    {"name": "Ingredient Name", "category": "Category", "confidence": 0.9, "uncertain": false, "quantity_hint": "Amount"}
  ],
  "note": "Optional short note"
}

Rules:
- DO NOT just return "Tomato" and "Onion" unless they are actually in the photo. 
- Look closely at the specific image provided and identify ONLY what you see.
- Categories: Vegetables, Fruits, Dairy, Meat, Seafood, Grains, Spices, Sauces, Snacks, Frozen, Leftovers, Other.
- Capitalize names simply (e.g., "Apple").
- If nothing is visible, return {"ingredients": [], "note": "No ingredients found"}.
"""
